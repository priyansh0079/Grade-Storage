...
from info.models import Dept, Class, Course, User, Student, Teacher, Assign, AssignTime, StudentCourse, Marks, \
    MarksClass, Attendance
...
class InfoTest(TestCase):
    ...
    def test_user_creation(self):
        ...
        s = Student(user=us, PRN='CS01', name='test')
        s.save()
    ...
    def create_class(self, id='CS5A', sem=5, batch='A'):
        dept = self.create_dept()
        return Class.objects.create(id=id, dept=dept, sem=sem, batch=batch)

    def test_class_creation(self):
        c = self.create_class()
        self.assertTrue(isinstance(c, Class))
        self.assertEqual(c.__str__(), "%s : %d %s" % (c.dept.name, c.sem, c.batch))
    ...
    def create_student(self, prn='CS01', name='samarth'):
        cl = self.create_class()
        u = self.create_user()
        return Student.objects.create(user=u, class_id=cl, PRN=prn, name=name)
    ...
    def test_index_student(self):
        self.client.login(username='test_user', password='test_password')
        s = Student.objects.create(user=User.objects.first(), PRN='test', name='test_name')
        response = self.client.get(reverse('index'))
    ...
    def test_no_attendance(self):
        s = self.create_student()
        self.client.login(username='test_user', password='test_password')
        response = self.client.get(reverse('attendance', args=(s.PRN,)))
    ...
    def test_no_attendance__detail(self):
        ...
        cr = self.create_course()
        self.client.login(username='test_user', password='test_password')
        resp = self.client.get(reverse('attendance_detail', args=(s.PRN, cr.id)))
